...
\begin{abstract}
We develop an auditable method for propagating lower bounds on the instantaneous spectral gap of affine adiabatic Hamiltonians. Weyl's inequalities yield a shift-invariant Lipschitz constant equal to the spectral width of $H_P-H_I$, improving uncentered norm estimates. An actual-step mark-and-probe continuation conditionally resolves all 20 $N=10$ and 19 of 20 $N=12$ Max-Cut instances. The continuous sweeps rely on floating-point eigenvalue indexing and are therefore conditional. Separately, exact-dyadic comparison-matrix checks certify four selected anchors. The study demonstrates a reproducible finite-size verification workflow; it does not establish asymptotic scaling, speedup, or a fully verified path.
\end{abstract}
...
The certification levels used here are local labels for the evidence recorded by this study. All continuous sweeps are Level~2 because ordinary sparse Ritz calculations do not independently certify eigenvalue indexing or outward-rounded residual arithmetic. Only the four points in Table~\ref{tab:level3-anchors} are Level~3 exact-dyadic certificates, and they do not establish a verified continuous path.

The 40-instance cohort contains only $N=10$ and $N=12$ problems and therefore supports neither asymptotic scaling claims nor conclusions about physically large systems. The fixed-grid and adaptive counts do not provide matched guarantees, so their comparison measures workload rather than speedup. One $N=12$ run retains a width-$0.001$ unresolved interval. Finally, Perron--Frobenius irreducibility and exact endpoint enumeration already establish nondegeneracy for the retained class; the experiment measures quantitative bound strength and cost, not discovery of gap positivity.
